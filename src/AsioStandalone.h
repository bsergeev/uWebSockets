...
struct Timer {
    ...
    void start(void (*cb)(Timer *), int first, int repeat) {
        ...
        asio_timer.async_wait([this, cb, repeat](const std::error_code &ec) {
...
struct Poll {
    ...
    Poll(Loop *loop, uv_os_sock_t fd) {
      ...
      socket = new asio::ip::tcp::socket(*loop);
    ...
    asio::ip::tcp::socket::native_handle_type getFd() {
      return (socket != nullptr)? socket->native_handle() : -1;
    }
    ...
    void start(Loop *, Poll *self, int events) {
        if (events & UV_READABLE) {
            socket->async_read_some(asio::null_buffers(), [self](std::error_code ec, std::size_t) {
                if (ec != asio::error::operation_aborted) {
        ...
        if (events & UV_WRITABLE) {
            socket->async_write_some(asio::null_buffers(), [self](std::error_code ec, std::size_t) {
                if (ec != asio::error::operation_aborted) {
